...
export default {
    ...
    formatDate(date, config) {
        if (!date) return ""
        if (config) {
            if (date.getTime) date = this.dateObj2str(date)
            ...
            if (config.todayAll) { //todayAll参数是获取今天日期时间格式：如2017-06-09 09:50:06
                return this.formatDateSymbol(date, "YYYY-MM-DD HH:mm:ss")
            }
            if (config.today) { //today参数是获取今天日期格式：如2017-06-09
                return this.formatDateSymbol(date, "YYYY-MM-DD")
            }
            if (config.monthDay) { //monthDay参数是获取年和月份：如2017-06
                return this.formatDateSymbol(date, "YYYY-MM")
            }
            if (config.todayAllCN) { //todayAll参数是获取今天日期时间格式：如2017年06月09日 09时50分06秒
                return this.formatDateCN(date, config.todayAllTextCN)
            }
    ...
    dateObj2str(date) {
        ...
        return y + this.leftAddZero(m) + this.leftAddZero(d) + this.leftAddZero(hh) + this.leftAddZero(mm) + this.leftAddZero(ss)
    },
    leftAddZero(number) {
        if (!number) return ''
    ...
    clearFormValue(vm, formName) {
        let formObj = vm[formName]
        for (name in formObj) {
            formObj[name] = ""
        }
    }
