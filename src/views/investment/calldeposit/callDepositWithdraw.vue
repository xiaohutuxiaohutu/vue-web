      ...
      <div class="p20">
        ...
        <div class="tableData mt20 printArea">
          <el-table  :data="tableData" stripe :default-sort="{prop:'depNo',order:'descending'}">
            ...
            <el-table-column label="通知种类" prop="accTp">
              <template scope="scope">
                <span>{{$tools.dict.callType(scope.row.accTp)}}</span>
              </template>
            </el-table-column>
            <el-table-column  label="通知存款本金" prop="accBalance" width="110">
              <template scope="scope">
            ...
            <el-table-column  label="开通渠道" prop="transChannel">
              <template scope="scope">
                ...
                <span v-if="scope.row.transChannel==''">网银</span>
              </template>
            </el-table-column>
            <el-table-column  label="操作">
              <template scope="scope">
                <span><a class="cOrange" v-if="scope.row.transChannel==''" @click="withdraw(scope.row)">支取</a></span>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  data() {
    return {
      warmTips: [
        '1.通知存款转活期的最低转存金额为10万元，留存通知存款账户余额不得低于50万元；',
        '2.企业通知存款账号和活期存款账号必须为相同开户名，且该两个账号均已加挂至网上银行；',
  ...
  methods: {
    ...
    withdraw(val) {
      ...
      val.withType = '1' //(全部支取/部分支取) 默认全部支取
      val.PNPAmt = '' //（支取金额）
      val.bsnCode = 'CB030202'
      val.orderNo = ''
      val.sequenceNo=''
      _this.$router.push({
        name: 'callWithdraw',
        params: $.extend({},val,{transInAccNo:'',transInAccName:'',openNode:'',openName:'',callOpenNode:_this.callOpenNode,callOpenBank:_this.callOpenBank,})}) //向history栈添加一个新记录
    },
