  ...
  export default {
    ...
    methods: {
      uKeySubmit(){
        ...
        //企业客户号|用户号|交易代码|通知存款账号|收款账号|类型|金额|支取方式|操作时间|网银流水号（需求中有“是否自动续存”字段）
      ...
      nextStep(){
        let _this = this
        _this.$tools.request(_this,"CB030202.do",_this.form).then(
          data => {
            if(data.body){
