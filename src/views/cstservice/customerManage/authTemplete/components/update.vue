        ...
        <tr>
          ...
          <td>
            <el-form-item prop="alias">
              <el-input v-model="form.alias" placeholder="模版名称" required></el-input>
            </el-form-item>
          </td>
          <td>模版类型：</td>
          <td>
        ...
        <el-table-column label="金额区间" :width="320" align="left">
          <template scope="scope">
            <el-form-item :prop="'amountRange.' + scope.$index + '.start'" :rules="currencyRules(scope.$index)">
              <ynet-currency-input class="w100" v-model="form2.amountRange[scope.$index].start" :change="startToEnd(scope.$index)" :showChinese="false" :disabled="form.areaType==0||scope.$index==0">
...
export default {
  ...
  created() {
    let _this = this
    _this.form = _this.tableData.templete
    // _this.form2.amountRange = _this.tableData.templeteArea
    ...
    _this.form2.amountRange = $.extend([], _this.tableData.templeteArea.sort(compare))
    let index = 0
