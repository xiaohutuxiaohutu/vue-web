  ...
  <div>
    <div class="fourForm tableForm mt20">
      ...
      <el-form ref="form" :model="form" :rules="rules" class="mb30">
        <el-row>
          <el-form-item label="转出母公司账号：" prop="payAccountNo" ref="payAccountNo" class="fullLine">
        ...
        <el-row>
          <el-form-item label="转出母公司账户名：" prop="payAccountName" class="fullLine">
      ...
      <el-row>
          <el-form-item label="转入子公司组别：" ref="" prop="acctGroup" class="fullLine">
  ...
  export default {
    ...
    methods: {
      ...
      retrieveAccts(){
        ...
        // _this.form.userId = list ? list.userNo : list.eCIFID
        for(let i = 0; i < operAccInfoList.length; i++){
          ...
          if(_type == '1' && _currency == 'CNY'){//集团转账权限的活期人民币账户
            ...
            let value=_acno
            if(_groupFlag == '1'){
              if(_right.substr(3,1)=='1' && _groupRight != '' && _groupRight.substr(1,1) == '1'){
                if(_auth == '2'){//操作员动账权限
                  // _this.recAccounts.push($.extend({}, operAccInfoList[i], {"value":value,"name":_name,"label":lable}))//子公司账号列表
                  _this.groupSessionAccs.push($.extend({}, operAccInfoList[i], {"value":value,"name":_name,"label":lable}))//子公司账号列表
                }
              }
            }else{
              _this.payAccounts.push($.extend({}, operAccInfoList[i], {"value":value,"name":_name,"label":lable}))//母公司账号列表
            }
          }
        }
      },
      ...
      onChangeRecAccountNo(acctNo){ //子公司账号选择事件
        let _this = this
        for(let i=0; i < _this.recAccounts.length; i++){
          if(acctNo == _this.recAccounts[i].value){
            _this.form.recName = _this.recAccounts[i].name || ''
      ...
      onChangePayAccountNo(acctNo){ //母公司账号选择事件
        let _this = this
      ...
      getGroupAccList(groupNo){
        ...
        if('otherAccNo' == _this.form.acctGroup){
          ...
          if(_this.groupInfoList.length <= 1){
            _this.recAccounts = _this.groupSessionAccs
            return
          }
        }else {
          queryData.reqTP = 'G'
              ...
              if(data.body.errorCode == '0'){
                ...
                if('otherAccNo' == _this.form.acctGroup){
                    ...
                    for(let i=0;i<payAccountsTemp.length;i++){
                          ...
                          if(flag1){
                              _this.recAccounts.push(payAccountsTemp[i])
                          }
                    }
      ...
      nextStep() {
        ...
        _this.$refs.form.validate((valid)  => {
          if (valid) {
            ...
            body.accountNo = _this.form.payAccountNo
            body.payAmount = _this.form.transAmount
            body.transDate = _this.$tools.utils.dateFormat(new Date(),"yyyyMMdd")
            _this.$tools.request(_this,"CB040203.do",body).then(
              data=>{
                let errorCode =data.body.errorCode
                let errorMsg =data.body.errorMsg
    ...
    mounted(){
      ...
      //       let lable=_lable+_name
      //       let value=_acno
      //       _this.payAccounts.push($.extend({}, operAccInfoList[i], {"value":value,"name":_name,"lable":lable}))
      //   }
      // }
    }
  }
