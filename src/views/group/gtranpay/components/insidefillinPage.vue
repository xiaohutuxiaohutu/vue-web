  ...
  <div>
    <div class="fourForm tableForm mt20">
      ...
      <el-form ref="form" :model="form" :rules="rules" class="mb30">
        <el-row>
          <el-form-item label="转出子公司组别：" ref="" prop="payAcctGroup" class="fullLine">
            <el-select v-model="form.payAcctGroup" placeholder="请选择" @change="getGroupPayAccList">
        ...
        <el-row>
          <el-form-item label="转出子公司账号：" prop="payAccountNo" ref="payAccountNo" class="fullLine">
            <el-select v-model="form.payAccountNo" placeholder="请选择" @change="onChangePayAccountNo">
                <template v-for="item in subAccounts">
      ...
      <!--<el-form ref="form" :model="form1" :rules="rules" >-->
        <el-row>
          <el-form-item label="转入子公司组别：" ref="" prop="recAcctGroup" class="fullLine">
            <el-select v-model="form.recAcctGroup" placeholder="请选择" @change="getGroupReciAccList">
              <template v-for="item in groupInfoList">
                  <el-option :label="item.groupName" :value="item.groupNo"></el-option>
              </template>
            </el-select>
        ...
        <el-row>
          <el-form-item label="转入子公司账号：" prop="recAccount" class="fullLine">
            <el-select v-model="form.recAccount" placeholder="请选择" @change="onChangeRecAccountNo">
              <el-option v-for="item in subAccounts1" :label="item.label" :value="item.value"></el-option>  
            </el-select>
            <span class="cOrange m15" v-if="recBalanceIndex==0">余额： {{$tools.utils.currencyFormat(recBalance)}}元</span>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item label="转入子公司账户名：" prop="recName" class="fullLine">
  ...
  export default {
    ...
    data() {
      return {
        use:false,
        rules: {
          recAcctGroup:[{ message: '请选择子公司组别', trigger:'change', required: true}],
          payAcctGroup:[{ message: '请选择子公司组别', trigger:'change', required: true}],
    ...
    methods: {
      ...
      retrieveAccts(){
        ...
        for(let i = 0; i < operAccInfoList.length; i++){
          ...
          if(_type == '1' && _currency == 'CNY'){//集团转账权限的活期人民币账户
            ...
            let value = _acno
            if(_groupFlag == '1'){
              if(_right.substr(3,1)=='1' && _groupRight != '' && _groupRight.substr(2,1) == '1'){
                if(_auth == '2'){//操作员动账权限
          ...
          data=>{
            ...
            if(errorCode =='0'){
              ...
              for(let i = 0; i < subAcctNoList.length; i++){
                ...
                let lable = _lable+_name
                let value = _acno
                _this.subAccounts.push({"value":value,"name":_name,"lable":lable})
              }
            }
          }
        )
      ...
      onChangeRecAccountNo(acctNo){ //母公司账号选择事件
        let _this = this
        for(let i=0; i<_this.subAccounts1.length; i++){
          if(acctNo == _this.subAccounts1[i].value){
            _this.form.recName = _this.subAccounts1[i].name
            _this.form.recopenNode = _this.subAccounts1[i].openBankNo || ''
            _this.form.recopenName = _this.subAccounts1[i].openBankName || ''
          }
        }
      ...
      onChangePayAccountNo(acctNo){ //子公司账号选择事件
        let _this = this
        for(let i=0; i<_this.subAccounts.length; i++){
          if(acctNo == _this.subAccounts[i].value){
            _this.form.payAccountName = _this.subAccounts[i].name
            _this.form.payNode = _this.subAccounts[i].openBankNo || ''
            _this.form.payOpenName = _this.subAccounts[i].openBankName || ''
          }
        }
      ...
      getGroupPayAccList(groupNo){
        ...
        if(groupNo){
          for(let i = 0; i < _this.groupInfoList.length; i++){
            ...
            if(groupNo == groupInfo.groupNo){
              _this.form.payAcctGroupDesc = groupInfo.groupName
            }
          }
        }
        ...
        _this.form.balance = ''
        let queryData = {}
        if('otherAccNo' == _this.form.payAcctGroup){
          queryData.reqTP = 'A'
          if(_this.groupInfoList.length <= 1){
            _this.subAccounts = _this.groupSessionAccs
            return
          }
        }else {
          queryData.reqTP = 'G'
          queryData.groupNo = _this.form.payAcctGroup
        }
              ...
              if(data.body.errorCode == '0'){
                ...
                if('otherAccNo' == _this.form.payAcctGroup){
                    ...
                    for(let i=0;i<payAccountsTemp.length;i++){
                          ...
                          if(flag1){
                              _this.subAccounts.push(payAccountsTemp[i])
                          }
                    }
      ...
      getGroupReciAccList(groupNo){
        ...
        if(groupNo){
          for(let i = 0; i < _this.groupInfoList.length; i++){
            let groupInfo = _this.groupInfoList[i]
            if(groupNo == groupInfo.groupNo){
              _this.form.recAcctGroupDesc = groupInfo.groupName
            }
          }
        }
        ...
        _this.recBalance = ''
        let queryData = {}
        if('otherAccNo' == _this.form.recAcctGroup){
          queryData.reqTP = 'A'
          if(_this.groupInfoList.length <= 1){
            _this.subAccounts1 = _this.groupSessionAccs
            return
          }
        }else {
          queryData.reqTP = 'G'
          queryData.groupNo = _this.form.recAcctGroup
        }
        _this.$tools.request(_this, "CB040406.do", queryData).then(
            data => {
              
              let reciAccountsTemp = _this.groupSessionAccs
              console.log('reciAccountsTemp',reciAccountsTemp)
              _this.subAccounts1 = []
              if(data.body.errorCode == '0'){
                // _this.acctGroupList = data.body.acctGroupList
                let acctGroupList = data.body.acctGroupList
                ...
                if('otherAccNo' == _this.form.recAcctGroup){
                    let flag1 = false
                    for(let i=0;i<reciAccountsTemp.length;i++){
                          for(let j=0;j<acctGroupList.length;j++){
                              if(acctGroupList[j].acctNo == reciAccountsTemp[i].accNo){
                                  flag1 = false
                                  break
                              }else if(Number(acctGroupList.length-1) == j){ //最后一条数据
                                  flag1 = true
                          ...
                          if(flag1){
                              _this.subAccounts1.push(reciAccountsTemp[i])
                          }
                    }
      ...
      nextStep() {
        ...
        _this.$refs.form.validate((valid)  => {
          if (valid) {
            if(_this.form.payAccountNo == _this.form.recAccount){
              _this.$alert('请选择不同账号', 'CHB', {confirmButtonText: '确定'});
              return
            }
            let body={}
            ...
            _this.$tools.request(_this,"CB040205.do",body).then(
              data=>{
                let errorCode =data.body.errorCode
                let errorMsg =data.body.errorMsg
    ...
    mounted(){
      ...
      // for(let i=0;i<operAccInfoList.length;i++){
      //   let _right=operAccInfoList[i].right
      //   if(_right.substr(0,1)==1){
      //     let _acno=operAccInfoList[i].accNo
      //       let _name=operAccInfoList[i].accName
      //       let _lable=_this.$tools.utils.mask(_acno,{beforeIndex:3,behindIndex:4})
      //       let lable=_lable+_name
      //       let value=_acno
      //       _this.recAccounts.push({"value":value,"name":_name,"lable":lable})
      //   }
      // }
    }
  }
